...
const Login = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      const result = await signIn("credentials", {
          ...
          username,
          password,
          redirect: false, // Prevent default redirect, we handle it manually
      });
  ...
  return (
    <div className="flex flex-col items-center gap-2 min-h-screen  -z-10 p-10">

        {/* generate a simple email username password form */}
        <div className='w-[350px] flex flex-col gap-2 items-center justify-center h-[450px] rounded-lg bg-sky-900 p-2'>

        {error && <p className="text-red-500">{error}</p>}
       <form onSubmit={handleSubmit} className='flex flex-col gap-2  justify-center items-center p-2 rounded-lg w-64 mb-4'>
        <div className='flex flex-col justify-start'>
        <label className='text-white' htmlFor="email">Email</label>
        <input type="email" id='email' className='bg-transparent outline-none border-2 border-white p-2 rounded-md'  onChange={(e) => setEmail(e.target.value)} />
        </div>

        <div className='flex flex-col justify-start'>
        <label className='text-white' htmlFor="username">Username</label>
        <input type="text" id='username' className='bg-transparent outline-none border-2 border-white p-2 rounded-md'  onChange={(e) => setUsername(e.target.value)} />
        </div>

        <div className='flex flex-col justify-start'>
        <label className='text-white' htmlFor="password">password</label>
        <input type="password" id='password' className='bg-transparent outline-none border-2 border-white p-2 rounded-md'  onChange={(e) => setPassword(e.target.value)} />
        </div>

        <button className='bg-white text-blue-500 px-6 py-2 rounded-lg font-semibold hover:bg-opacity-90 w-full mt-4 text-lg' type="submit">Login</button>
       </form>
